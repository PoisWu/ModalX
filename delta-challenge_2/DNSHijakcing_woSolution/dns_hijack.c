...
#define s_iphdr 20
#define s_udphdr 8
#define s_dns_header 12
#define s_question 4
#define s_r_element 10
#define s_sockaddr_in 16
...
int main(int argc, char *argv[])
{
	...
	*(dev_name + strlen(dev_name) - 1) = '\0'; //the pcap_open_live don't take the last \n in the end
	...
	memset(&to,0,s_sockaddr_in);
    to.sin_family=2;//AF_INET;
...
//	PrintData(buffer, size);
	...
	//	parse the dns query
	parse_dns_query(dns_buff, queries, answers, auth, addit,0);
	...
	uint8_t *pointer_msg = send_buf+s_iphdr+s_udphdr+s_dns_header;
	...
	for(int i=0;i<qd_count;i++){
		...
		q_queris->qclass=htons(CLASS_IN);
		q_queris->qtype=htons(TYPE_A);
		pointer_msg+=s_question;
	}
	for(int i=0;i<qd_count;i++){
		res_record r;
		r.name = pointer_msg;
	...
	/**********dns header*************/
	dns_header *dnshdr = (dns_header*)(send_buf +s_iphdr +s_udphdr);
    //TODO
	memcpy(dnshdr, dns_recv, s_dns_header);
	dnshdr->qr=1;
	dnshdr->an_count=dns_recv->qd_count;

	/****************UDP header********************/
	struct udphdr *out_udphdr = (struct udphdr*)(send_buf +s_iphdr);
	...
	out_iphdr->check=checksum((unsigned short *)out_iphdr, 20);	
