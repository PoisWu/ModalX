...
| -------------256(buf)------| ----------128(greeting_txt)--------| -----some registers store(like rbp)------|------6(return addr)-----|
I dont really know how many byte taken by registers, so i make a for loop to try all the possible offset. and i found the return_addr is at the 12+140=152 after greeting_txt,
whitch means that there are 3(24/3) registers stored in the stack.
...
For reason of symetry. i wrote 2 sub_thread fd_write and fd_read whitch write and read the file discriptor sockfd seperiately.
