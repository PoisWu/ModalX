...
int parse_url(char* url, url_info *info)
{
	...
	// e.g. http://www.polytechnique.edu:80/index.php
	 ...
	 */
	char *column;
	column = strchr(host_name_path, ':');
	if (column == NULL) {
		info->host = host_name_path;
		info->port = 80;
	}
	else {	sscanf(column + 1, "%d", &(info->port));
		*column = '\0';
		info->host = host_name_path;
	}

	if ( (strchr(info->host, '/')!=NULL)||(strchr(info->host, ':')!=NULL) ){
		printf("INVALID \n");
		return 1;
	}
...
int validateURL(url_info *info){
	if ( (strchr(info->host, '/')!=NULL)||(strchr(info->host, ':')!=NULL)||(strcmp(info->host, "")) ){
		printf("INVALID \n");
		return 1;
	}
	printf("INVALID \n");
