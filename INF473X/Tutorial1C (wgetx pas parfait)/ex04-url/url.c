...
int parse_url(char* url, url_info *info)
{
	...
	if(strcmp(protocol, "http")){
		perror("PARSE_URL_PROTOCOL_UNKNOWN");
		return 3;
	}
	/*
	 * To be completed:
	 ...
	 */
	path_i = strstr(host_name_path, "/");
	if(path_i == NULL) {
		perror("PARSE_URL_NO_SLASH");
		return 1;		
	}
	info->path = path_i+1;//take the / out
	/*
	 * To be completed:
	 ...
	 */
	port_i = strstr(host_name_path, ":");
	if(port_i==NULL || port_i>path_i){
		info->host = strtok(host_name_path,"/");	
		info->port = 80;
	}else
	{
		info->port = strtol(port_i+1, &ptr, 10);
		if(info->port==0 || ptr!=path_i){
				perror("PARSE_URL_INVALID_PORT");
				return 2;
		}
		info->host = strtok(host_name_path,":");
	}
...
int validateURL(char* url){
	url_info info;
	int test;
	test = parse_url(url, &info);
	if (!test) return 0;
	if(strcmp(info.protocol, "http")){
		perror("PARSE_URL_PROTOCOL_UNKNOWN");
		return 0;
	}
	if(strlen(info.host)==0) return 0;
	if(strstr(info.host,":")!=NULL || strstr(info.host,"/")!=NULL) return 0;
	//port integer is granted by the definition of port in our structure
	//same as path
	return 1;
}
